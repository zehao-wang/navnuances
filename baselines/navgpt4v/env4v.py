...
class Simulator(object):
    ...
    def __init__(
        ...
        # Setup Actual Matterport Simulator
        WIDTH = 512
        HEIGHT = 512
        VFOV = math.radians(90) # math.radians(60)
        ...
        sim.initialize() # NOTE: if initilaization failed by EGL error, try to use -X option in ssh on the machine has a display, it should works
        self.sim_env = sim
    ...
    def newEpisode(
        ...
        self.sim_env.newEpisode([self.scan_ID], [self.viewpoint_ID], [self.heading], [self.elevation])
        # Load navigable dict
    ...
    def get_current_view(self):
        pil_list = []
        for i in range(8):
            state = self.sim_env.getState()[0]
            cv_img = np.array(state.rgb, copy=True)
            
            cv_img = cv2.putText(cv_img, str(i), (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 4, TEXT_COLOR_BORDER, thickness=12)
            cv_img = cv2.putText(cv_img, str(i), (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 4, TEXT_COLOR, thickness=8)
            
            pil_list.append(Image.fromarray(cv_img[:,:,::-1]))

            print(f"Index {i}: ", [loc.viewpointId for loc in state.navigableLocations[1:]])

            self.sim_env.makeAction([0], [math.radians(45)], [0])

        print()
        for i, img in enumerate(pil_list):
            img.save(f"./cache/{i}.png")
        
        # NOTE: concate views into one
        margin = 10
        cv_img_cat = np.ones((512 * 3 + margin * 2, 512 * 3 + margin * 2, 3), dtype=np.uint8) * 255
        ...
        for i, pil_img in enumerate(pil_list):
            ...
            cv_img_cat[
                h_start*512 + margin*h_start: h_start*512 + margin*h_start + 512,  
                w_start*512 + margin*w_start: w_start*512 + margin*w_start + 512
            ] = np.asarray(pil_img)

        pil_img_cat = Image.fromarray(cv_img_cat)
        pil_img_cat = pil_img_cat.resize((512, 512), Image.LANCZOS)
        pil_img_cat.save(f"./cache/cat.png")
        # pil_list = [pil_img_cat]

        return pil_list
    
    def getState(self) -> dict:
        rgb_pil = self.get_current_view()
        self.state = {
            'scanID': self.scan_ID,
    ...
    def makeAction(self, next_viewpoint_ID):
        ...
        print('\033[1;32m [INFO]\033[0m Move to: ', [self.viewpoint_ID, self.heading, self.elevation])
        self.sim_env.newEpisode([self.scan_ID], [self.viewpoint_ID], [self.heading], [self.elevation])
        self.getCandidate()
...
class EnvBatch(object):
    ...
    def getStates(self):
        ...
        for i, sim in enumerate(self.sims):
            ...
            pano_views = [] 
            for img in state['pil_img']:
                pano_views.append(encode_image(pil_image=img))

            feature['detail_vis'] = pano_views 
            # feature['detail'] = pano_views # replace detail
            # feature['summary'] = curr_img # replace summary
